...
function PdfAnnotatorModal({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`https://reussite-qcmss-1nc7.onrender.com/api/users/annotation?userId=${userId}&fileUrl=${encodeURIComponent(fileUrl)}`);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch('https://reussite-qcmss-1nc7.onrender.com/api/users/annotation', {
        method: 'PUT',
  ...
  const generateModifiedPDFInBrowser = async () => {
    ...
    try {
      ...
      const uploadRes = await fetch('https://reussite-qcmss-1nc7.onrender.com/api/upload', {
        method: 'POST',
        body: formData
      });
      if (!uploadRes.ok) {
  ...
  const handleCreateModified = async () => {
    ...
    try {
      const res = await fetch('https://reussite-qcmss-1nc7.onrender.com/api/users/custom-file', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId,
