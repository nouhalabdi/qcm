...
function AppContent({ darkMode, toggleDarkMode, user }) {
  ...
  useEffect(() => {
    ...
    const newSocket = io('https://reussite-qcmss-1nc7.onrender.com');
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`https://reussite-qcmss-1nc7.onrender.com/api/notifications?userId=${user._id}`);
